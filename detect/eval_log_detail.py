...
"""
Compute detection breakdown statistics and update eval_log_detail.json.

This script analyzes evaluation results by comparing ground truth errors (from synth JSON files)
with detected findings. It computes macro and micro detection rates across different dimensions:
corruption_type, difficulty, location, and needs_cross_section.

The script extracts ground truth from synth JSON files (prioritizing apply_results with applied==True),
matches them with evaluation results, and aggregates statistics both per-paper (macro) and pooled (micro).

Usage Examples:
    # Compute breakdown and update eval_log_detail.json
...
Key Arguments:
    --root_dir: Root directory containing paper_synth_*.json files. Default: data/ICML_30
    --detect_model: Model tag used for detection. 
    --eval_model: LLM model tag used for evaluation/adjudication. Default: gpt-5-2025-08-07
    --eval_mode: Evaluation mode name (determines eval directory prefix). 
    --synth_model: Filter papers by filename containing this substring. 
    --max_papers: Limit the number of synth files to process. Default: None (no limit)
    --remove: Remove the matching run from eval_log_detail.json and exit. Default: False

Output:
    - Statistics are written to: <root_dir>/eval_log_detail.json
    - The log file contains aggregated detection rates (macro and micro) broken down by:
...
def compute_eval_path(
    ...
    # <paper_dir>/detect/<eval_mode>_eval/<detect_model>/<synth_stem>/eval_{eval_model_tag}.json
    return paper_dir / "detect" / f"{eval_mode}_eval" / detect_model / synth_stem / f"eval_{_tag(eval_model)}.json"
...
def main():
    ...
    ap.add_argument("--root_dir", type=str, default="data/ICML_30",
                    help="Root directory containing paper_synth_*.json files")
    ap.add_argument("--detect_model", type=str, default="gpt-5-2025-08-07",
                    help="Model tag used for detection")
    ap.add_argument("--eval_model", type=str, default="gpt-5-2025-08-07",
                    help="LLM model tag used for evaluation/adjudication")
    ap.add_argument("--eval_mode", type=str, default="standard_wo_memory",
                    help="Evaluation mode name (determines eval directory prefix, e.g., 'standard' => standard_eval)")
    ap.add_argument("--synth_model", type=str, default="gpt-5-2025-08-07",
                    help="Filter papers by filename containing this substring")
    ap.add_argument("--max_papers", type=int, default=None,
                    help="Limit the number of synth files to process (default: None, no limit)")
    ap.add_argument("--remove", action="store_true", default=False,
                    help="Remove the matching run from eval_log_detail.json and exit")
