...
# -*- coding: utf-8 -*-
"""
batch_parse_and_review.py

批量解析论文文件夹：
- 输入一个根目录（如 download/downloads/ICLR_2025_oral）
- 自动在其子目录中查找 paper.pdf
- 调用异步解析流水线，生成 paper_parse_origin.json 写回子目录
- 并行执行（可调并发），具备断点续传（已存在的 paper_parse_origin.json 会跳过）
- 带指数回退重试与详细日志

依赖：
  - pillow, tqdm, litellm, openai, (可选) opencv-python
  - 你已有的 LlamaParse 服务封装：llama_cloud_services.LlamaParse
环境变量：
...
try:
    import cv2  # 可选：无 bbox 时用于模板匹配兜底
except Exception:
    cv2 = None
...
def _bbox_contains(b_big, b_small, tol: int = 2, over_ratio: float = 0.10) -> bool:
    """
    b_small 是否被 b_big 包含；允许像素容差，且四向超出比例 <= over_ratio 则视为包含
    """
    x0, y0, x1, y1 = b_big
    x2, y2, x3, y3 = b_small
...
def filter_subimages(
    ...
    """
    以包含关系去重子图：按面积由大到小保留；被包含且明显更小的丢弃
    """
    enriched = []
    for it in items:
        p = Path(it[path_key])
...
async def parse_one_pdf(
    ...
    """
    解析单个 PDF 并写出为 out_json_path（paper_parse_origin.json）
    若 out_json_path 已存在，直接返回 ok=True（断点续传）
    """
    if out_json_path.exists():
        return ParseResult(ok=True, out_path=out_json_path)

    # 写一个 .inprogress 标记，避免重复
    ...
    # 延迟 import，避免未设置环境时报错
    from llama_cloud_services import LlamaParse

    # 设置环境变量（litellm/openai在外层通过环境变量读取）
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            for page in result.pages:
                ...
                # 2) 仅嵌入真正的 manuscript 图像（layout_picture）
...
def find_paper_pdfs(root_dir: Path) -> List[Path]:
    """
    在 root_dir 的一级子目录中查找 paper.pdf
    形如：root_dir/<paper_folder>/paper.pdf
    """
    pdfs = []
...
def main():
    ...
    parser.add_argument(
        "--root_dir",
        type=str, default="/mnt/parallel_ssd/home/zdhs0006/mlrbench/download/downloads/ICLR_30",
        help="根目录，例如 downloads/ICLR_2025_oral（其子目录内含 paper.pdf）",
    )
    parser.add_argument(
        "--concurrency", "-j",
        type=int, default=10,
    ...
    parser.add_argument(
        ...
        help=f"用于 OCR 校正/图片判别的模型名（默认 {DEFAULT_MODEL}）",
    )
