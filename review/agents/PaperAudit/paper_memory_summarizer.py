# agents/PaperAudit/paper_memory_summarizer.py
import json
...
class PaperMemorySummarizer(BaseAgent):
    ...
    Output: Plain text with high information density, used by downstream reviewers to quickly recall paper content.
    """
...
"""
    ...
    async def run(self, pdf_text: Any, *, section_titles: list[str] | None = None) -> str:
        ...
        Args:
            pdf_text: Full paper text already extracted from PDF (no longer base64)

        Returns:
            Plain text memory (single string, no longer SSE / AsyncGenerator)
        """
