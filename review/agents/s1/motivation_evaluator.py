...
"""
    ...
    async def run(self, paper_summary: str) -> str:
        ...
        Args:
            pdf_content: paper content in well-formed structured text（可以是全文，也可以是 paper_memory）

        Returns:
            纯文本 motivation evaluation 字符串
        """
...
if __name__ == "__main__":
    ...
    evaluator = MotivationEvaluator()
    
    with open(PDF_FILE, "rb") as f:
        pdf_base64 = base64.b64encode(f.read()).decode('utf-8')
    pdf_text = evaluator.extract_pdf_text_from_base64(pdf_base64)
    print(f"PDF text length: {len(pdf_text)} characters")
    # print(pdf_text[:10])  # Print first 10 characters of the PDF text
    
    sample_text = "<tag>Hello World</tag> Some other text <tag>Another Tag</tag>"
    print(extract_tag(sample_text, "tag"))
    
    # asyncio.run(evaluator._extract_keywords(pdf_text))
    # results = asyncio.run(evaluator._search_with_retry(query="Transformer self-attention multi-head attention sequence transduction neural networks", max_results=10))
    # asyncio.run(evaluator._most_similar_works(pdf_text, results))
    report = asyncio.run(evaluator.run(pdf_text))
    print(report)
    # python -c "from dotenv import load_dotenv; load_dotenv(); import runpy; runpy.run_module('agents.s1.motivation_evaluator', run_name='__main__')"